  ...
  const handleSignUp = async () => {
    try {
      ...
      if (!allowedImageTypes.includes(tutorImage.type)) {
        toast.error(
          "Invalid image format. Please upload a valid image file (JPEG or PNG).",
        );
        return;
      }
      ...
      let res = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL || "https://localhost:7010/api"}/Account/tutor-register`,
        formData,
        {
          headers: {
  ...
  const handleSendVerifyCode = async () => {
    try {
      ...
      let res1 = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL || "https://localhost:7010/api"}/Account/send-verification-code`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
    ...
    <>
      <Navbar expand="lg" className="bg-body-tertiary">
        <Container fluid>
          ...
          <Navbar.Collapse id="navbarScroll">
            ...
            >
              <Nav.Link as={Link} to={"/"}>
                <span className="navBarContent">Find tutor</span>
              </Nav.Link>
              <Nav.Link as={Link} to={"/sign-up-tutor"}>
                <span className="navBarContent">Become a tutor</span>
              </Nav.Link>
              <NavDropdown
                title={<span className="navBarContent">Contact us</span>}
                id="navbarScrollingDropdown"
              >
                <NavDropdown.Item>
                  <Link as={Link} to={"/send-report"}>
                    Send report
                  </Link>
                </NavDropdown.Item>
      ...
      <div className="signup-container col-3 sign-up-form">
        ...
        <div className="haveacc">
          ...
          <Link className="forgot-password" as={Link} to={"/login"}>
            {" "}
            Log in
          </Link>
        </div>
        ...
        <input
          ...
          className="form-control"
          value={tutorAge}
          onChange={(event) => setTutorAge(event.target.value)}
        />
        ...
        <input
          ...
          onChange={(event) => setTutorDescription(event.target.value)}
        />

        <div className="text">Picture</div>
        <div>
          <input type="file" onChange={handlePreviewAvartar} />
          {tutorImage && <img src={tutorImage.preview} alt="" width="50%" />}
        </div>
