  ...
  const handEditProfile = async () => {
    ...
    if (confirmed) {
      if (userInformation.roleId === "TUTOR") {
        ...
        let url =
          `${process.env.REACT_APP_API_BASE_URL || "https://localhost:7010/api"}/Account/update-tutor?accountId=` +
          userInformation.id;
        ...
        password
          ? formData.append("PasswordModel.CurrentPassword", password)
          : formData.append("PasswordModel.CurrentPassword", "");
        newPassword
          ? formData.append("PasswordModel.Password", newPassword)
          : formData.append("PasswordModel.Password", "");
        confirmNewPassword
          ? formData.append("PasswordModel.ConfirmPassword", confirmNewPassword)
          : formData.append("PasswordModel.CurrentPassword", "");
        ...
        try {
          let url =
            `${process.env.REACT_APP_API_BASE_URL || "https://localhost:7010/api"}/Account/update-learner?accountId=` +
            userInformation.id;
          let res = await axios.put(url, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
    ...
    <>
      ...
      >
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => handEditProfile()}>
            Edit Profile
          </Button>
        </Modal.Footer>
      </Modal>
